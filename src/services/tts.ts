...
const PIPER_VOICES_DIR = join(TELECLAW_ROOT, "piper-voices");
const PIPER_VENV = join(TELECLAW_ROOT, "rvc-env");
 ...
 * Uses custom voices from ~/.teleclaw/piper-voices/
...
async function generatePiperTTS(text: string, voice: string): Promise<TTSResult> {
  const tempDir = join(tmpdir(), "teleclaw-tts");
