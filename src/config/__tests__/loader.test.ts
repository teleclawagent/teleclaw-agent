...
describe("Config Loader", () => {
  ...
  describe("loadConfig - default values", () => {
    it("should apply default values for optional fields", () => {
      ...
      // Telegram defaults
      expect(config.telegram.session_name).toBe("teleclaw_session");
      expect(config.telegram.dm_policy).toBe("allowlist");
...
`;
  ...
  describe("loadConfig - environment variable overrides", () => {
    it("should override api_key with TELECLAW_API_KEY", () => {
      writeTestConfig(MINIMAL_CONFIG);
      process.env.TELECLAW_API_KEY = "sk-ant-override-key";
    ...
    it("should override webui enabled with TELECLAW_WEBUI_ENABLED=false", () => {
      writeTestConfig(FULL_CONFIG); // WebUI enabled in config
      process.env.TELECLAW_WEBUI_ENABLED = "false";

      const config = loadConfig(TEST_CONFIG_PATH);
      expect(config.webui.enabled).toBe(false);
    });

    it("should override webui port with TELECLAW_WEBUI_PORT", () => {
      writeTestConfig(MINIMAL_CONFIG);
      process.env.TELECLAW_WEBUI_PORT = "9999";

      const config = loadConfig(TEST_CONFIG_PATH);
      expect(config.webui.port).toBe(9999);
    });

    it("should ignore invalid TELECLAW_WEBUI_PORT", () => {
      writeTestConfig(MINIMAL_CONFIG);
      process.env.TELECLAW_WEBUI_PORT = "not_a_number";

      const config = loadConfig(TEST_CONFIG_PATH);
      expect(config.webui.port).toBe(7777); // Default
    });

    it("should override webui host with TELECLAW_WEBUI_HOST", () => {
      writeTestConfig(MINIMAL_CONFIG);
      process.env.TELECLAW_WEBUI_HOST = "0.0.0.0";
  ...
  describe("loadConfig - path expansion", () => {
    it("should expand tilde in telegram session_path", () => {
      ...
      expect(config.telegram.session_path).toBe(join(homedir(), ".teleclaw"));
    });
    ...
    it("should expand tilde in storage paths", () => {
      ...
      expect(config.storage.sessions_file).toBe(join(homedir(), ".teleclaw/sessions.json"));
      expect(config.storage.memory_file).toBe(join(homedir(), ".teleclaw/memory.json"));
    });
...
`;
  ...
  describe("configExists", () => {
    ...
    it("should expand tilde paths", () => {
      ...
      try {
        const homePath = join(TEST_DIR, ".teleclaw-test-config.yaml");
        writeTestConfig(MINIMAL_CONFIG, homePath);
        expect(configExists("~/.teleclaw-test-config.yaml")).toBe(true);
  ...
  describe("getDefaultConfigPath", () => {
    it("should return the default config path", () => {
      ...
      expect(defaultPath).toContain(".teleclaw");
...
`;
      ...
      process.env.TELECLAW_API_KEY = "sk-env-override";
      process.env.TELECLAW_WEBUI_ENABLED = "true";
