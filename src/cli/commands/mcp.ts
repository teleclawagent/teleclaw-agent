 ...
 *
 *   teleclaw mcp add <package> [args...]   — add an MCP server
 *   teleclaw mcp remove <name>             — remove an MCP server
 *   teleclaw mcp list                      — list configured MCP servers
 */
...
export async function mcpAddCommand(
  ...
  console.log(`\n   Restart teleclaw to connect: teleclaw start`);
}
...
export async function mcpListCommand(options: { config?: string }): Promise<void> {
  ...
  if (entries.length === 0) {
    ...
    console.log("\n  Add one: teleclaw mcp add @modelcontextprotocol/server-filesystem /tmp");
    return;
  }
