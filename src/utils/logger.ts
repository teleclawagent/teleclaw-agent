...
function resolveLevel(): LogLevel {
  // TELECLAW_LOG_LEVEL takes priority
  const explicit = process.env.TELECLAW_LOG_LEVEL?.toLowerCase();
  if (explicit && isValidLevel(explicit)) {
    return explicit;
  }

  // Backward compat: TELECLAW_LOG=verbose → debug
  if (process.env.TELECLAW_LOG === "verbose") {
    return "debug";
  }
...
const usePretty = process.env.TELECLAW_LOG_PRETTY !== "false";
 ...
 * Note: pretty mode is controlled by TELECLAW_LOG_PRETTY env var only
...
export function initLoggerFromConfig(logging: { level?: string }): void {
  ...
  if (!process.env.TELECLAW_LOG_LEVEL && !process.env.TELECLAW_LOG) {
