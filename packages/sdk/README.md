...
```bash
npm install @teleclaw-agent/sdk
```
...
A Teleclaw plugin is a module that exports a `tools` function and, optionally, `manifest`, `start`, and `migrate`.

```typescript
import type { PluginSDK, SimpleToolDef, PluginManifest } from "@teleclaw-agent/sdk";
...
Place the compiled plugin at `~/.teleclaw/plugins/<name>/index.js` and register it in `config.yaml`:
...
```typescript
import type { PluginCallbackEvent } from "@teleclaw-agent/sdk";
...
```typescript
import { SDK_VERSION } from "@teleclaw-agent/sdk";
// "1.0.0"
```
