...
This guide walks through building, testing, and distributing plugins for Teleclaw Agent. Plugins extend the agent with new tools that the LLM can invoke, event hooks for real-time message processing, and background tasks.
...
A Teleclaw plugin is a JavaScript module (ESM) placed in `~/.teleclaw/plugins/`. It exports one required item (`tools`) and several optional lifecycle hooks. The platform discovers plugins at startup, validates them, and integrates their tools into the LLM's available tool set.
...
- The official SDK package is `@teleclaw-agent/sdk` on npm
...
Create `~/.teleclaw/plugins/hello.js`:
...
> **Path restriction**: `sendStory` only accepts files from `/tmp`, `Downloads/`, `Pictures/`, `Videos/`, or the teleclaw workspace directory. Other paths are rejected for security.
...
Secrets are stored in `~/.teleclaw/plugins/data/<plugin>.secrets.json` with `0600` permissions.
...
The database file is created at `~/.teleclaw/plugins/data/<plugin-name>.db`. Each plugin gets its own isolated database -- plugins cannot access each other's data.
...
When enabled, the platform watches `~/.teleclaw/plugins/` for file changes using `chokidar`. On detecting a change:
...
cp -r path/to/teleclaw-plugins/plugins/casino ~/.teleclaw/plugins/casino
```
...
The simplest approach: share your plugin file or directory. Users place it in `~/.teleclaw/plugins/`.
...
```json
{
  "name": "teleclaw-plugin-my-feature",
  "version": "1.0.0",
  "main": "index.js",
  "peerDependencies": {
    "@teleclaw-agent/sdk": ">=1.0.0"
  }
}
```
...
npm init -y
npm install teleclaw-plugin-my-feature
```

Then create an `index.js` that re-exports:

```javascript
export { tools, manifest, migrate, start, stop } from "teleclaw-plugin-my-feature";
```
...
# Deploy the compiled .js to ~/.teleclaw/plugins/
cp dist/index.js ~/.teleclaw/plugins/my-plugin/index.js
```

Install `@teleclaw-agent/sdk` as a dev dependency for type definitions:

```bash
npm install -D @teleclaw-agent/sdk
```
...
12. **`sendStory` path restrictions**: Only files from `/tmp`, `Downloads/`, `Pictures/`, `Videos/`, or the teleclaw workspace are allowed. Other paths are rejected for security.
